...
int main()
{
    float theta = 0.5, time_count;
    double x, y, phi, caA2, ct2 = cos(theta) * cos(theta), st2 = sin(theta) * sin(theta), sct = cos(theta) * sin(theta), averMeanvalue, sumAvalue = 0, sumMeanvalue;
    ...
    for (int t = 1; t < 10000000; t++)
    {
        ...
        for (unsigned int n = N; n--;)
        {
            ...
            if (n == 0)
            {
                phiA[n] = ct2 * x0A[n] - sct * x0B[N - 1] + st2 * x0A[N - 1] + sct * x0B[n];
                phiB[n] = ct2 * x0B[n] - sct * x0A[n] + st2 * x0B[n + 1] + sct * x0A[n + 1];
            }
            else if (n == N - 1)
            {
                phiA[n] = ct2 * x0A[n] - sct * x0B[n - 1] + st2 * x0A[n - 1] + sct * x0B[n];
                phiB[n] = ct2 * x0B[n] - sct * x0A[n] + st2 * x0B[0] + sct * x0A[0];
            }
            else
            {
                phiA[n] = ct2 * x0A[n] - sct * x0B[n - 1] + st2 * x0A[n - 1] + sct * x0B[n];
        ...
        for (unsigned int n = N; n--;)
        {
            ...
            // caA2 = caA2 * caA2;
            x0A[n] = (cos(caA2) + I * sin(caA2)) * phiA[n];
    ...
    printf("%f\n", time_count);
    FILE *fp1;
    fp1 = fopen("Variance_g_1_N_5000_M_1_T_7_s_05", "w");
